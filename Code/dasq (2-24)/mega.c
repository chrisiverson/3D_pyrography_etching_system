...
if (actually_reset && image_choice == 1 && drawing_image == 0 && image_done == 0 && automatic == 1)
{
  ...
  z_with_object_ht = z_with_object_ht + 5;
        ZD = 10;
        //pen_down = 1;
        //Serial.println(pen_down);
}

/*if (automatic = 1 && image_done == 1 && !z_endstopvar_max && !y_endstopvar_max)
{
      automatic = 0;
}*/
...
if (drawing_image && XR == 0 && XL == 0 && YU == 0 && YD == 0 && image_done == 0)
{
    ...
    if ((XL > 1 || XR > 1 || YD > 1 || YU > 1) && pen_down)
    {
      //ZD = 0;
      //ZU = 5;
      pen_down = 0;
    ...
    //else if (!pen_down && (XL == 1 || XR == 1 || YD == 1 || YU == 1))
    //{
      //pen_down = 1;
      //Serial.println(pen_down);
    //}
