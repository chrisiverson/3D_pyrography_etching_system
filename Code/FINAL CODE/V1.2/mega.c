...
// 0 - Hand Control, 1 - Image data, 2 - No calibration*beta*
int image_choice = 1;

int ht_chnge = 15;
int image_gain = 13;
char move_now = 1;
char drawing_image = 0;
int current_step = 0;
char image_done = 0;
char z_confirm = 0;
char point_check = 0;
...
void loop(){
 ...
 buttonState = digitalRead(buttonPin);
  ...
  if (UD < (UPMID+60) && (UD > (UPMID-60)))// original  = (UPMID-10)
      UD = UPMID;
  if (LR < (LRMID+60) && (LR > (LRMID-60)))
      LR = LRMID;
...
///////////////////////AUTO-IMAGE DRAWING/////////////////////////////
if (!z_endstopvar_max && !y_endstopvar_max && percentage_complete == 250  && automatic)
{
...
if (drawing_image && XR == 0 && XL == 0 && YU == 0 && YD == 0 && image_done == 0)
{
    ...
    //Serial.println(XR1);
    if ((XL1 > image_gain || XR1 > image_gain || YD1 > image_gain || YU1 > image_gain) && pen_down && move_now && !point_check)
    {
      ZD = 0;
      ZU = 300;
      pen_down = 0;
    ...
    else if ((XL1 > image_gain || XR1 > image_gain || YD1 > image_gain || YU1 > image_gain) && !pen_down && move_now && !point_check)
    {
       ZD = 300;
       ZU = 0;
       pen_down = 1;
